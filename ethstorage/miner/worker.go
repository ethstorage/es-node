...
func (w *worker) newWorkLoop() {
	...
	for {
		...
		case ts := <-sumCh:
			tskSumsList := tskSums[ts.shard][ts.block]
			if tskSums == nil {
				tskSumsList := make([]*taskSum, 0)
				tskSums[ts.shard][ts.block] = tskSumsList
			}
			tskSumsList = append(tskSumsList, ts)
			// all threads collected
			if len(tskSumsList) == int(w.config.ThreadsPerShard) {
				var noncesTried uint64
				var timeUsed float64
				for i := 0; i < len(tskSumsList); i++ {
					noncesTried += tskSumsList[i].noncesTried
					timeUsed += tskSumsList[i].timeUsed
				}
				w.lg.Info("Mine tasks done", "shard", ts.shard, "threads", w.config.ThreadsPerShard, "block", ts.block,
					"totalNoncesTried", noncesTried,
					"timeUsedInAverage", fmt.Sprintf("%.1fs", timeUsed/float64(w.config.ThreadsPerShard)),
				)
				delete(tskSums[ts.shard], ts.block)
			}
		case <-w.exitCh:
...
func (w *worker) taskLoop(taskCh chan *taskItem, sumCh chan *taskSum) {
	defer w.wg.Done()
